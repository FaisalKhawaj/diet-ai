...
export function RecipeProvider({ children }: { children: ReactNode }) {
  ...
  const { current, setCanNext, total, setTotal, type } = useStepper();
  const [showRecipeAI, setShowRecipeAI] = useState(false);
  const [showAddRecipe, setShowAddRecipe] = useState(false);
  const [showDietRecipe, setShowDietRecipe] = useState(false);

  // ⬇️ Only set 7 steps if the active flow is recipe
  useEffect(() => {
    if (type === "recipe" && total !== 7) setTotal(7);
  }, [type, total, setTotal]);
  ...
  const addIngredientToInclude = (name: string) =>
    setQa((p) => ({
      ...p,
      includeIngredients: Array.from(
        new Set([...(p.includeIngredients ?? []), name.trim()])
      ),
    }));
  ...
  const removeIngredientFromInclude = (name: string) =>
    setQa((p) => ({
      ...p,
      includeIngredients: (p.includeIngredients ?? []).filter(
        (x) => x !== name
      ),
    }));
  ...
  const addIngredientToExclude = (name: string) =>
    setQa((p) => ({
      ...
      excludeIngredients: Array.from(
        new Set([...(p.excludeIngredients ?? []), name.trim()])
      ),
    }));
  ...
  const removeIngredientFromExclude = (name: string) =>
    setQa((p) => ({
      ...p,
      excludeIngredients: (p.excludeIngredients ?? []).filter(
        (x) => x !== name
      ),
    }));
  ...
  const setTargetMacros = (patch: Partial<TargetMacros>) =>
    setQa((p) => ({
      ...p,
      targetMacros: { ...(p.targetMacros ?? {}), ...patch },
    }));
  ...
  const submit = () => {
    ...
    const allGood = (
      Object.keys(validators) as unknown as QuestionSteps[]
    ).every((s) => isStepValid(s));
    if (!allGood) return;
  ...
  const value = useMemo<CtxType>(
    ...
    [qa, showRecipeAI, showAddRecipe, showDietRecipe, showSetting]
  );
