import Onboarding1 from "./onboarding1.png";
import Onboarding2 from "./onboarding2.png";
import Onboarding3 from "./onboarding3.png";
import Onboarding4 from "./onboarding4.png";

import ThreeMonths from './3months.png';
import SixMonths from './6months.png';
import Camera from './camera.png';
import Carrot from './carrot.png';
import CarrotOutlined from './carrotoutlined.png';
import Crown from './crown.png';
import CupDietAi from './cupdietai.png';
import DocRec from './docrecipe.png';
import GreenCheckMark from './greencheckmark.png';
import Lock from './lock.png';
import OneWeek from './oneweek.png';
import SettingImg from './settingpic.png';
import Speed from './speed.png';
import Spoon from './spoon.png';
import WelcomeImg from "./welcome.png";

export { Camera, Carrot, CarrotOutlined, Crown, CupDietAi, DocRec, GreenCheckMark, Lock, Onboarding1, Onboarding2, Onboarding3, Onboarding4, OneWeek, SettingImg, SixMonths, Speed, Spoon, ThreeMonths, WelcomeImg };

